 ...
 * raylib 5.6
 *
 *   Example contributed by Zero (@zerohorsepower) and reviewed by Ramon
 * Santamaria (@raysan5)
 *
...
typedef struct Bullet {
  ...
  Vector2 acceleration; // Amount of pixels to be incremented to position every
                        // frame
...
int main(void) {
  ...
  // Bullets definition
  Bullet *bullets =
      (Bullet *)RL_CALLOC(MAX_BULLETS, sizeof(Bullet)); // Bullets array
  int bulletCount = 0;
  int bulletDisabledCount =
      0; // Used to calculate how many bullets are on screen
  int bulletRadius = 10;
  ...
  int angleIncrement = 5; // After spawn all bullet rows, increment this value
                          // on the baseDirection for next the frame
  ...
  // NOTE: This is done to improve the performance, since DrawCircle() is very
  // slow
  ...
  bool drawInPerformanceMode =
      true; // Switch between DrawCircle() and DrawTexture()
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    // New bullets will replace the old ones that are already disabled due to
    // out-of-screen
    ...
    if (spawnCooldownTimer < 0) {
      ...
      for (int row = 0; row < bulletRows; row++) {
        if (bulletCount < MAX_BULLETS) {
          ...
          // Bullet speed*bullet direction, this will determine how much pixels
          // will be incremented/decremented from the bullet position every
          // frame. Since the bullets doesn't change its direction and speed,
          // only need to calculate it at the spawning time
          // 0 degrees = right, 90 degrees = down, 180 degrees = left and 270
          // degrees = up, basically clockwise Case you want it to be
          // anti-clockwise, add "* -1" at the y acceleration
          bullets[bulletCount].acceleration =
              (Vector2){bulletSpeed * cosf(bulletDirection * DEG2RAD),
    ...
    // Input logic
    if ((IsKeyPressed(KEY_RIGHT) || IsKeyPressed(KEY_D)) && (bulletRows < 359))
      bulletRows++;
    if ((IsKeyPressed(KEY_LEFT) || IsKeyPressed(KEY_A)) && (bulletRows > 1))
      bulletRows--;
    ...
    if ((IsKeyPressed(KEY_DOWN) || IsKeyPressed(KEY_S)) &&
        (bulletSpeed > 0.50f))
      bulletSpeed -= 0.25f;
    if (IsKeyPressed(KEY_Z) && (spawnCooldown > 1))
      spawnCooldown--;
    ...
    if (drawInPerformanceMode) {
        ...
        if (!bullets[i].disabled) {
          DrawTexture(
              ...
              (int)(bullets[i].position.y -
                    bulletTexture.texture.height * 0.5f),
              bullets[i].color);
        }
      }
    } else {
      ...
      for (int i = 0; i < bulletCount; i++) {
        // Do not draw disabled bullets (out of screen)
        if (!bullets[i].disabled) {
          DrawCircleV(bullets[i].position, (float)bulletRadius,
                      bullets[i].color);
    ...
    DrawText("- Space (Hold): Change the angle increment", 40, 100, 10,
             LIGHTGRAY);
    DrawText("- Enter: Switch draw method (Performance)", 40, 120, 10,
             LIGHTGRAY);
    ...
    DrawText(TextFormat("[ FPS: %d, Bullets: %d, Rows: %d, Bullet speed: %.2f, "
                        "Angle increment per frame: %d, Cooldown: %.0f ]",
                        GetFPS(), bulletCount - bulletDisabledCount, bulletRows,
                        bulletSpeed, angleIncrement, spawnCooldown),
             155, 420, 10, GREEN);
